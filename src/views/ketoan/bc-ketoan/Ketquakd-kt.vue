  ...
  methods: {
    mySearchNoZero2() {
      if (this.todosSave.length > 0) {
        this.ketquakds = this.todosSave // hoàn lại
        this.todosSave = []
      } else {
        let temp = this.ketquakds.filter((row) => {
          return (
        ...
        this.todosSave = this.ketquakds // Lưu
        this.ketquakds = temp
      }
    },
    ...
    updateTodo() {
      ...
      apiService
        .patch(`${this.model}/${ketquakd.id}`, ketquakd)
        .then((r) => {
          if (r.status === 200) {
