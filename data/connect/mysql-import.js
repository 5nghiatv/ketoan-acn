...
class Importer {
  ...
  setEncoding(encoding) {
    var supported_encodings = ['utf8', 'ucs2', 'utf16le', 'latin1', 'ascii', 'base64', 'hex']
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        await slowLoop(files, (file, index, next) => {
          ...
          // ---------- Nghia moi them
          this._importSingleFile(streamNew, file)
            .then(() => {
              next()
            })
            .catch((err) => {
   ...
   * @returns {Promise}
   */
  _importSingleFile(streamNew, fileObj) {
    return new Promise((resolve, reject) => {
      // ---------- Nghia moi them
      fileObj.size = streamNew ? streamNew.length : fileObj.size
...
class queryParser extends stream.Writable {
  ...
  async _write(chunk, enc, next) {
    ...
    // Nghia Moi Them
    // chunk = this.streamNew ? this.streamNew : chunk ;
    // fs.writeFileSync('_'+ Math.floor(Date.now() / 1000)+'.sql',chunk);
